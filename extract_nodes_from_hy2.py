...
TEST_MAX_NODES = int(os.getenv("TEST_MAX_NODES", 200))
TEST_TIMEOUT = float(os.getenv("TEST_TIMEOUT", 2))
...
def load_invalid_urls():
    ...
    try:
        ...
        debug_logs.append(f"Debug {len(invalid_urls)} invalid URLs loaded")
...
def parse_node(node):
    try:
        if node.startswith(("ss://", "hysteria2://", "trojan://", "vless://")):
            match = re.match(r'^(?:ss|hysteria2|trojan|vless|v|https)://(?:[^@]+@)?([^:]+):(\d+)', node)
            if match:
...
if GITHUB_TOKEN:
    ...
    debug_logs.append("警告：未发现 BOT 环境变量")
    ...
    try:
        if proxy_type == 'ss':
            ...
            vmess_config = {
                ...
                "type": proxy.get('headerType', 'none "tls": proxy.get('tls', ''),
        ...
        async with session.get(raw_url, headers=headers, timeout=15) as response:
            ...
            for match in protocol_matches:
                ...
                if protocol_pattern.match(node) and len(node) > 10:
                    ...
                    debug_logs.append(f"提取明文协议节点: {node[:50]}...")

            # 提取 Base64
            base64_matches = base64_pattern.findall(content)
            debug_logs.append(f"找到 {len(base64_matches)} 个 Base64 字符串")
            for b64_str in base64_matches:
                if any(param in b64_str.lower() for param in ['encryption=', 'security=', 'sni=', 'type=', 'mode=', 'serviceName=', 'fp=', 'pbk=', 'sid=']):
                    debug_logs.append(f"跳过非 Base64 参数: {b64_str[:50]}...")
                    continue
                try:
            ...
            debug_logs.append(f"URL {index+1}/{len(urls)} 处理完成，耗时 {elapsed:.2f} seconds秒")
...
async def test_and_save_nodes():
    ...
    tasks = [test_node_async(node, timeout=2) for node in nodes_to_test]
    ...
    for i, (node, result) in enumerate(zip(nodes_to_test, results)):
        ...
        debug_logs.append(f"提取并测试 {len(valid_protocol_nodes)} 个有效协议节点")

    # 主流程
    async def main():
        async with aiohttp.ClientSession() as session:
            await check_rate_limit(session)
            urls_set = list(set(urls))[:50] # 限制为 50 个 URL 以加快调试
            total_urls = len(urls_set)
            tasks = []

        debug_logs.append("Phase 1: Starting to extract nodes from all URLs...")
        ...
        for i, result in enumerate(results):
            ...
            debug_logs.append(f"Extracted {len(protocol_nodes_set)} raw protocol nodes saved to {temp_nodes_file} (pending test)")
            ...
            with open(debug_log_file, "w", encoding="utf-8") as f:
                f.write("\n".join(debug_logs))
            debug_logs.append(f"Debug log saved to {debug_log_file}")
