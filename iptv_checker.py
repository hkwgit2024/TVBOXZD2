#专注于频道的可用性检查，并会读取 iptv_processor.py 生成的最终 IPTV 文件，然后对其中的每个频道进行连接测试。它还会维护 URL 状态，以便在后续检查中跳过近期失败的链接。
import os
import re
import subprocess
import socket
...
def setup_logging(config):
    ...
    log_file = config['logging']['log_file'].replace('.log', '_checker.log') # 独立的日志文件
...
def write_channels_to_file(file_path, data_list):
    ...
    参数:
        file_path: 输出文件路径
        data_list: 包含频道数据的列表 (name, url)
    """
    try:
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        with open(file_path, 'w', encoding='utf-8') as file:
            for name, url in sorted(data_list, key=lambda x: x[0]):
                file.write(f"{name},{url}\n")
        logging.info(f"写入 {len(data_list)} 个频道到 {file_path}")
    except Exception as e:
...
def check_webrtc_url(url, timeout):
    ...
    try:
        ...
        if not parsed_url.scheme == 'webrtc':
            logging.debug(f"非 WebRTC 协议: {url}")
            return False
...
def process_single_channel_line(channel_line, url_states):
    ...
    参数:
        ...
        url_states: URL 状态字典
    返回:
        元组 (响应时间, (频道名称, URL))，若无效则返回 (None, None)
    """
...
def check_channels_multithreaded(channel_lines, url_states, max_workers=CONFIG['network']['channel_check_workers']):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            try:
                elapsed_time, channel_info = future.result() # channel_info 是 (name, url) 元组
                if elapsed_time is not None and channel_info is not None:
                    results.append((elapsed_time, channel_info))
            except Exception as exc:
                logging.warning(f"处理频道行时发生异常: {exc}")
    logging.warning(f"多线程频道检查完成，发现 {len(results)} 个有效频道")
    return results
...
@performance_monitor
def save_checked_channels_to_final_file(final_iptv_file_path, valid_channels, url_states):
    """将检查后的有效频道保存到最终的 IPTV 文件中
    参数:
        final_iptv_file_path: 最终 IPTV 文件路径
        valid_channels: 列表，包含有效的 (name, url) 元组
        url_states: URL 状态字典
    """
    # 重新分类以便按类别写入
    categorized_channels, uncategorized_channels, final_ordered_categories = categorize_channels(valid_channels)

    try:
        os.makedirs(os.path.dirname(final_iptv_file_path), exist_ok=True)
        with open(final_iptv_file_path, "w", encoding='utf-8') as iptv_list_file:
            ...
            if uncategorized_channels:
                # 再次检查配置是否允许写入未分类频道到主文件
                # 默认情况下，iptv_processor.py将未分类频道写入单独的文件
                # 在这里，为了确保主文件完整性，如果未分类频道存在，也可以选择性写入
                # 考虑到用户需求是“检查iptv_processor.py的结果”，这里可以选择性地写入
                # 假设我们希望将所有有效频道写入主文件，包括未分类的。
                # 如果config.yaml中有uncategorized_channels_in_main_file: true，则写入
                # 否则，只处理已分类的。为了简化，这里直接写入。
                iptv_list_file.write("其他频道,#genre#\n")
                for name, url in sorted(uncategorized_channels, key=lambda x: x[0]):
                    iptv_list_file.write(f"{name},{url}\n")
        logging.warning(f"检查后的有效频道已保存到: {final_iptv_file_path}")
    except Exception as e:
        logging.error(f"保存检查后的文件 '{final_iptv_file_path}' 失败: {e}")
...
def check_main():
    ...
    包含以下步骤：
    1. 加载 URL 状态
    2. 读取 iptv_processor.py 生成的最终 IPTV 列表
    3. 多线程检查频道有效性
    4. 保存更新后的 URL 状态
    5. 保存检查后的有效频道到新的文件（或覆盖原文件）
    """
    logging.warning("开始执行 IPTV 频道检查脚本")
    ...
    # 步骤 2：读取 iptv_processor.py 生成的最终 IPTV 列表
    # 我们读取IPTV_LIST_PATH，并跳过头部信息和#genre#行
    channels_to_check_lines = []
    try:
        with open(IPTV_LIST_PATH, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith(('更新时间', '#genre#')):
                    channels_to_check_lines.append(line)
        logging.warning(f"从 '{IPTV_LIST_PATH}' 读取 {len(channels_to_check_lines)} 个频道待检查")
    except FileNotFoundError:
        logging.error(f"错误：未找到 IPTV 列表文件 '{IPTV_LIST_PATH}'。请先运行 iptv_processor.py。")
        exit(1)
    except Exception as e:
        logging.error(f"读取 IPTV 列表文件 '{IPTV_LIST_PATH}' 失败: {e}")
        exit(1)

    if not channels_to_check_lines:
        logging.warning("IPTV 列表中没有需要检查的频道，退出。")
        save_url_states_local(url_states) # 确保即使没有频道也要保存状态
        exit(0)

    # 步骤 3：多线程检查频道有效性
    # valid_channels_with_speed 是 (elapsed_time, (name, url)) 的列表
    valid_channels_with_speed = check_channels_multithreaded(channels_to_check_lines, url_states)
    
    # 提取 (name, url) 元组
    valid_channels_only = [channel_info for _, channel_info in valid_channels_with_speed]
    
    logging.warning(f"检查完成，总计 {len(valid_channels_only)} 个有效频道")

    # 步骤 4：保存更新后的 URL 状态
    save_url_states_local(url_states)
    logging.warning("最终频道检查状态已保存")

    # 步骤 5：保存检查后的有效频道到最终文件
    # 这里将直接覆盖原文件，以确保最终文件只包含有效频道
    save_checked_channels_to_final_file(IPTV_LIST_PATH, valid_channels_only, url_states)

    total_elapsed_time = time.time() - total_start_time
    logging.warning(f"IPTV 频道检查脚本完成，总耗时 {total_elapsed_time:.2f} 秒")
