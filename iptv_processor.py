...
from tenacity import retry, stop_after_attempt, wait_fixed, retry_if_exception_type # 即使简化也保留tenacity，因为它在fetch_url_content_with_retry中使用
import json
import hashlib
...
from datetime import datetime, timedelta # 用于 URL 状态管理
...
GITHUB_RAW_CONTENT_BASE_URL = "https://raw.githubusercontent.com/{REPO_OWNER}/{REPO_NAME}/main" # 用于fetch_from_github, 理论上这个脚本不需要它了，但为了兼容性可以保留
...
def write_sorted_channels_to_file(file_path, data_list):
    ...
    new_channels = set()
    for _, line in data_list: # data_list 格式可能是 (sort_key, "name,url")
        if ',' in line:
...
# --- URL 处理和频道提取函数 (保留主要逻辑，移除 ffprobe 相关) ---
@performance_monitor
...
def extract_channels_from_url(url, url_states, source_tracker):
    ...
    try:
        start_time = time.time()
        text = fetch_url_content_with_retry(url, url_states) # 内部包含 ETag/Last-Modified/Content-Hash 检查
        if text is None:
            logging.info(f"URL {url} 无新内容或获取失败，跳过提取")
            return []
        ...
        if extension in [".m3u", ".m3u8"]:
            ...
            logging.info(f"不支持的文件扩展名或非 M3U/TXT/CSV 内容，跳过: {url}")
            return []
            ...
            if "," in line and "://" in line:
                ...
                if '#' in channel_address_raw:
                    ...
                    if channel_url and pre_screen_url(channel_url):
                        ...
                        logging.info(f"跳过无效或预筛选失败的频道 URL: {channel_url}")
            elif re.match(r'^[a-zA-Z0-9+.-]+://', line): # 单独的 URL 行
...
def fetch_url_content_with_retry(url, url_states):
    ...
    if content_cache is not None and url in content_cache: # 检查 content_cache 是否已初始化
...
def pre_screen_url(url):
    ...
    try:
        parsed_url = urlparse(url)
        # 简化版只检查 http/https，因为不再依赖 ffprobe
        if parsed_url.scheme not in ['http', 'https']:
            logging.info(f"预筛选过滤（不支持的协议，仅支持 HTTP/HTTPS）: {url}")
            return False
...
# --- 频道有效性检查函数 (简化版，仅 HTTP HEAD 检查) ---
@performance_monitor
def check_http_head_url(url, timeout):
    """简单检查 HTTP/HTTPS URL 是否可达（仅 HEAD 请求）"""
    try:
        response = session.head(url, timeout=timeout, allow_redirects=True)
        return 200 <= response.status_code < 400
    except requests.exceptions.RequestException as e:
        logging.info(f"HTTP HEAD URL 检查失败: {url} - {e}")
        return False

@performance_monitor
def check_channel_validity_and_speed_simplified(channel_name, url, url_states, timeout=CONFIG['network']['check_timeout']):
    """简化版检查单个频道的有效性和速度，仅 HTTP HEAD 检查"""
    current_time = datetime.now()
    ...
    try:
        if url.startswith("http"): # 只检查 HTTP/HTTPS
            is_valid = check_http_head_url(url, timeout)
        else:
            logging.info(f"频道 {channel_name} 的协议不支持 HTTP HEAD 检查 (简化模式): {url}")
            if url not in url_states:
                url_states[url] = {}
            url_states[url]['last_checked_protocol_unsupported_simplified'] = current_time.isoformat()
        ...
        if is_valid:
            ...
            url_states[url]['last_stream_checked'] = current_time.isoformat()
            logging.info(f"频道 {channel_name} ({url}) 检查成功 (简化模式)，耗时 {elapsed_time:.0f} ms")
            ...
            url_states[url]['last_stream_checked'] = current_time.isoformat()
            logging.info(f"频道 {channel_name} ({url}) 检查失败 (简化模式)")
            return None, False
    except Exception as e:
        if url not in url_states:
            url_states[url] = {}
        ...
        url_states[url]['stream_fail_count'] = current_url_state.get('stream_fail_count', 0) + 1
        url_states[url]['last_stream_checked'] = current_time.isoformat()
        logging.info(f"检查频道 {channel_name} ({url}) 错误 (简化模式): {e}")
        return None, False
...
def process_single_channel_line(channel_line, url_states):
    ...
    if len(parts) == 2:
        ...
        url = url.strip()
        # 调用简化版检查函数
        elapsed_time, is_valid = check_channel_validity_and_speed_simplified(name, url, url_states)
        if is_valid:
...
def check_channels_multithreaded(channel_lines, url_states, max_workers=CONFIG['network']['channel_check_workers']):
    ...
    logging.warning(f"开始多线程检查 {total_channels} 个频道的有效性和速度 (简化模式)")
...
def merge_local_channel_files(local_channels_directory, output_file_name, url_states):
    ...
    logging.warning(f"总计 {len(channels_for_checking_lines)} 个唯一频道待检查和过滤 (简化模式)")
    ...
    try:
        ...
        logging.warning(f"所有频道列表文件合并、去重、分类完成 (简化模式)，输出保存到: {output_file_name}")
    except Exception as e:
...
def main():
    """主函数，执行 IPTV 处理流程"""
    logging.warning("开始执行 IPTV URL 发现和处理脚本")
    ...
    # 步骤 2：从 GitHub 自动发现新 URL (此功能依然保留)
    ...
    # 10：清理临时文件（保留未分类文件）(保留)
    try:
        temp_files = ['iptv.txt', 'iptv_speed.txt'] # 这些是旧的，可能不会生成了
        for temp_file in temp_files:
            if os.path.exists(temp_file):
                os.remove(temp_file)
                logging.info(f"移除临时文件 '{temp_file}'")
        temp_dir = CONFIG['output']['paths']['channels_dir'] # 分类文件目录
        if os.path.exists(temp_dir):
            # 保留 uncat 文件
            files_to_remove = [f_name for f_name in os.listdir(temp_dir) if f_name.endswith('_iptv.txt')]
            for f_name in files_to_remove:
                os.remove(os.path.join(temp_dir, f_name))
                logging.info(f"移除临时分类频道文件 '{f_name}'")

            if not os.listdir(temp_dir): # 如果目录为空，则删除
                os.rmdir(temp_dir)
                logging.info(f"移除空目录 '{temp_dir}'")
        
        # 确认未分类文件路径是否存在，并告知用户会保留
        uncategorized_file = CONFIG['output']['paths']['uncategorized_channels_file']
        if os.path.exists(uncategorized_file):
            logging.warning(f"保留未分类文件 '{uncategorized_file}'")
        else:
            logging.info(f"未分类文件 '{uncategorized_file}' 未生成或不存在，无需保留")

        # 检查并移除 urls.txt 文件，如果最终不需要在仓库中保留它
        # 考虑到你后续要commit urls.txt，所以这里不移除它
        # if os.path.exists(URLS_PATH):
        #     logging.info(f"保留 '{URLS_PATH}' 文件")

    except Exception as e:
        logging.error(f"清理临时文件失败: {e}")

    total_elapsed_time = time.time() - total_start_time
    logging.warning(f"IPTV URL 发现和处理脚本完成，总耗时 {total_elapsed_time:.2f} 秒")
