        ...
        if scheme == "ss":
            try:
                ...
                if method not in VALID_SS_CIPHERS:
                    print(f"❌ 错误：SS 链接使用了不支持的加密方法：{method} (链接: {link})")
                    return None
...
def generate_plaintext_node_link(proxy: dict) -> str | None:
    ...
    if p_type == "ss":
        ...
        if all([server, port, uuid]):
            ...
            if ws_headers_host:
                vmess_obj["host"] = ws_headers_host
            if tls:
                vmess_obj["tls"] = "tls"
            if servername:
...
async def test_clash_meta_nodes(clash_core_path: str, config_path: str, api_port: int = 9090, retries: int = 3) -> list:
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if clash_process:
                if clash_process.returncode is None:
...
async def main():
    ...
    for proxy in all_proxies:
        key_parts = (
            ...
            proxy.get("ws-path"),
            json.dumps(proxy.get("ws-headers", {}), sort_keys=True)
        )
        key = hash(key_parts)
