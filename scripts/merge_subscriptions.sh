...
for i in "${!URLS[@]}"; do
  ...
  yq eval '.[].tls = (.tls | if (. == "true" or . == "True") then true elif (. == "false" or . == "False" or . == "" or . == null) then false else false end)' temp_proxies_for_yq_fix.yaml > temp_proxies_fixed.yaml
  if [ $? -ne 0 ]; then
    echo "错误: 修复 TLS 配置失败"
    echo "调试: 显示 temp_proxies_for_yq_fix.yaml 前10行内容..."
    head -n 10 temp_proxies_for_yq_fix.yaml
    exit 1
  fi
  ...
  # 验证 temp_proxies_fixed.yaml 是否有效
  if [ ! -s temp_proxies_fixed.yaml ]; then
    echo "错误: temp_proxies_fixed.yaml 为空，跳过"
    continue
  fi

  # 合并到 ALL_PROXIES_COLLECTED
  echo "合并修复后的代理到 $ALL_PROXIES_COLLECTED..."
  yq eval-all 'select(fileIndex == 0) + select(fileIndex == 1)' "$ALL_PROXIES_COLLECTED" temp_proxies_fixed.yaml > temp_merged.yaml
  if [ $? -ne 0 ] || [ ! -s temp_merged.yaml ]; then
    echo "错误: 合并代理失败"
    echo "调试: 显示 temp_proxies_fixed.yaml 前10行内容..."
    head -n 10 temp_proxies_fixed.yaml
  ...
  mv temp_merged.yaml "$ALL_PROXIES_COLLECTED"
  proxy_found=true
done
...
if [ ! -s "$FINAL_CONFIG" ] || [ $(yq eval '.proxies | length' "$FINAL_CONFIG") -eq 0 ]; then
  echo "错误: 生成的 $FINAL_CONFIG 为空或无有效 proxies"
  echo "调试: 显示 $FINAL_CONFIG 完整内容..."
  cat "$FINAL_CONFIG"
  exit 1
fi
