...
# 配置日志
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s', filename='iptv_crawler.log')
...
def fetch_from_github(file_path_in_repo):
    ...
    try:
        ...
        logging.debug(f"成功从 GitHub 获取文件：{raw_url}")
...
def save_to_github(file_path_in_repo, content, commit_message):
    ...
    try:
        ...
        response.raise_for_status()
        logging.debug(f"成功保存文件到 GitHub：{file_path_in_repo}")
        return True
    except requests.exceptions.RequestException as e:
...
def load_config():
    """从本地仓库加载并解析 YAML 配置文件。"""
    config_path = os.getenv("CONFIG_PATH", "config/config.yaml")
    logging.info(f"尝试加载本地配置文件：{config_path}")
    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            config = yaml.safe_load(f)
            logging.info(f"本地配置文件解析成功：{config}")
            return config
    except FileNotFoundError:
        logging.error(f"本地配置文件不存在：{config_path}")
        exit(1)
    except yaml.YAMLError as e:
        logging.error(f"YAML 解析错误：{e}，文件：{config_path}")
        exit(1)
    except Exception as e:
        logging.error(f"加载本地配置文件失败：{e}，文件：{config_path}")
        exit(1)
