...
def setup_logging(config):
    ...
    logging.info(f"日志系统初始化完成。日志文件: {log_file}，日志级别设置为: {logging.getLevelName(logger.level)}") # <-- 添加的日志
    return logger
...
def load_config(config_path="config/config.yaml"):
    ...
    try:
        with open(config_path, 'r', encoding='utf-8') as file:
            ...
            logging.info(f"成功加载配置文件: {config_path}") # <-- 添加的日志
            return config_data
    except FileNotFoundError:
        logging.critical(f"错误：未找到配置文件 '{config_path}'") # <-- 添加的日志
        exit(1)
    except yaml.YAMLError as e:
        logging.critical(f"错误：配置文件 '{config_path}' 格式错误: {e}") # <-- 添加的日志
        exit(1)
    except Exception as e:
        logging.critical(f"错误：加载配置文件 '{config_path}' 失败: {e}") # <-- 添加的日志
        exit(1)
...
logging.info(f"日志级别已根据配置文件设置为: {log_level_from_config}") # <-- 添加的日志


# 检查环境变量 GITHUB_TOKEN
GITHUB_TOKEN = os.getenv('BOT')
if not GITHUB_TOKEN:
    logging.critical("错误：未设置环境变量 'BOT'。请在GitHub Secrets中配置 BOT 环境变量。") # <-- 添加的日志
    exit(1)
...
UNCATEGORIZED_CHANNELS_FILE = CONFIG['output']['paths']['uncategorized_channels_file'] # 新增
CHANNELS_DIR = CONFIG['output']['paths']['channels_dir'] # 新增
...
if CONFIG['url_state']['cache_enabled']:
    ...
    content_cache = TTLCache(maxsize=1000, ttl=CONFIG['url_state']['cache_ttl']) # maxsize硬编码在原始代码中，这里保持
    logging.info(f"URL内容缓存已启用，最大大小: 1000，TTL: {CONFIG['url_state']['cache_ttl']}秒。") # <-- 添加的日志
...
logging.info(f"Requests会话已配置，连接池大小: {pool_size}，重试次数: {CONFIG['network']['requests_retry_total']}") # <-- 添加的日志
...
def performance_monitor(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logging.info(f"性能监控：函数 '{func.__name__}' 耗时 {elapsed_time:.2f} 秒") # <-- 添加的日志
        return result
    return wrapper
...
def fetch_from_github(file_path_in_repo):
    ...
    try:
        logging.info(f"正在从 GitHub 获取文件: {file_path_in_repo} (URL: {raw_url})") # <-- 添加的日志
        response = session.get(raw_url, headers=headers, timeout=10)
        response.raise_for_status()
        logging.info(f"成功从 GitHub 获取文件: {file_path_in_repo}") # <-- 添加的日志
        return response.text
    except requests.exceptions.RequestException as e:
        logging.error(f"错误：从 GitHub 获取 {file_path_in_repo} 失败: {e}") # <-- 添加的日志
        return None
...
def get_current_sha(file_path_in_repo):
    ...
    try:
        ...
        logging.debug(f"获取 {file_path_in_repo} 的 SHA 值: {sha}") # <-- 添加的日志
        return sha
    except requests.exceptions.RequestException as e:
        logging.debug(f"获取 {file_path_in_repo} 的 SHA 值失败（可能不存在）: {e}") # <-- 添加的日志
        return None
...
def save_to_github(file_path_in_repo, content, commit_message):
    ...
    if sha:
        payload["sha"] = sha
        logging.info(f"文件 {file_path_in_repo} 已存在，正在更新。") # <-- 添加的日志
    else:
        logging.info(f"文件 {file_path_in_repo} 不存在，正在创建。") # <-- 添加的日志

    try:
        response = session.put(api_url, headers=headers, json=payload)
        response.raise_for_status()
        logging.info(f"成功保存 {file_path_in_repo} 到 GitHub。") # <-- 添加的日志
        return True
    except requests.exceptions.RequestException as e:
        logging.error(f"错误：保存 {file_path_in_repo} 到 GitHub 失败: {e} (Response: {e.response.text if e.response else 'N/A'})") # <-- 添加的日志
        return False
...
def read_txt_to_array_local(file_name):
    ...
    try:
        ...
        logging.info(f"成功从本地文件 '{file_name}' 读取 {len(lines)} 行数据。") # <-- 添加的日志
        return lines
    except FileNotFoundError:
        logging.warning(f"文件 '{file_name}' 未找到，返回空列表。") # <-- 添加的日志
        return []
    except Exception as e:
        logging.error(f"读取文件 '{file_name}' 失败: {e}") # <-- 添加的日志
        return []
...
def read_existing_channels(file_path):
    ...
    try:
        ...
        logging.debug(f"从 '{file_path}' 读取了 {len(existing_channels)} 个现有频道用于去重。") # <-- 添加的日志
    except FileNotFoundError:
        logging.debug(f"去重文件 '{file_path}' 未找到，假定无现有频道。") # <-- 添加的日志
        pass
    except Exception as e:
        logging.error(f"读取文件 '{file_path}' 进行去重失败: {e}") # <-- 添加的日志
...
def write_sorted_channels_to_file(file_path, data_list):
    ...
    # 原始代码中这里是 `all_channels = existing_channels | new_channels` 
    # 但是在后面的循环中，它只写入了 `if (name, url) not in existing_channels:` 的部分
    # 这意味着它只追加了新发现的频道，而没有重新写入所有频道。
    # 为了保持原始逻辑，我们只处理 `new_channels` 并追加。
    
    channels_to_append = []
    for name, url in sorted(list(new_channels), key=lambda x: x[0]): # 对新频道进行排序
        if (name, url) not in existing_channels: # 确保只添加真正的新频道
    ...
    try:
        ...
        logging.debug(f"追加 {len(channels_to_append)} 个新频道到 {file_path}") # <-- 添加的日志
    except Exception as e:
        logging.error(f"追加写入文件 '{file_path}' 失败: {e}") # <-- 添加的日志
...
def get_url_file_extension(url):
    ...
    try:
        ...
        logging.debug(f"获取 URL 扩展名失败: {url} - {e}") # <-- 添加的日志
        return ""
...
def convert_m3u_to_txt(m3u_content):
    ...
    logging.debug(f"M3U内容转换为TXT格式，提取了 {len(txt_lines)} 条记录。") # <-- 添加的日志
    return '\n'.join(txt_lines)
...
def clean_url_params(url):
    ...
    try:
        ...
        logging.debug(f"清理 URL 参数失败: {url} - {e}") # <-- 添加的日志
        return url
...
def extract_channels_from_url(url, url_states, source_tracker):
    ...
    try:
        ...
        if text is None:
            logging.debug(f"未从URL {url} 获取到内容或内容未变更。") # <-- 添加的日志
            return []

        extension = get_url_file_extension(url).lower()
        if extension in [".m3u", ".m3u8"]:
            text = convert_m3u_to_txt(text)
        elif extension in [".ts", ".flv", ".mp4", ".hls", ".dash"]: # 直接流媒体URL
            ...
            if pre_screen_url(url):
                ...
                logging.debug(f"提取单一流: {channel_name},{url} from {url}") # <-- 添加的日志
            return extracted_channels
        elif extension not in [".txt", ".csv", ""]: # 允许没有扩展名（常见于API响应），但原始代码这里是 `extension not in [".txt", ".csv"]`
            # 原始代码没有考虑空扩展名，这里按照原始代码逻辑
            logging.debug(f"不支持的文件扩展名或缺失: {extension} for URL {url}") # <-- 添加的日志
            return []
        ...
        for line in lines:
            ...
            if "," in line and "://" in line: # name,url 格式
                parts = line.split(',', 1)
                if len(parts) != 2:
                    logging.debug(f"跳过无效频道行（格式错误）: {line}") # <-- 添加的日志
                    continue
                ...
                if not re.match(r'^[a-zA-Z0-9+.-]+://', channel_address_raw):
                    logging.debug(f"跳过无效频道 URL（无有效协议）: {line}") # <-- 添加的日志
                    continue

                if '#' in channel_address_raw: # 处理带#号分隔符的URL列表
                    ...
                    for channel_url_single in url_list:
                        ...
                        if channel_url and pre_screen_url(channel_url):
                            ...
                            logging.debug(f"跳过无效或预筛选失败的频道 URL: {channel_url_single}") # <-- 添加的日志
                else: # 单一URL
                    ...
                    if channel_url and pre_screen_url(channel_url):
                        ...
                        logging.debug(f"跳过无效或预筛选失败的频道 URL: {channel_address_raw}") # <-- 添加的日志
            elif re.match(r'^[a-zA-Z0-9+.-]+://', line): # 纯URL格式
                channel_name = f"Stream_{channel_count + 1}" # 默认名称
                ...
                if channel_url and pre_screen_url(channel_url):
                    ...
                    logging.debug(f"跳过无效或预筛选失败的单一 URL: {line}") # <-- 添加的日志
        logging.debug(f"成功从 {url} 提取 {channel_count} 个频道。") # <-- 添加的日志
    except Exception as e:
        logging.error(f"从 {url} 提取频道失败: {e}") # <-- 添加的日志
    return extracted_channels
...
def load_url_states_local():
    ...
    try:
        ...
        logging.info(f"从 '{URL_STATES_PATH}' 加载了 {len(url_states)} 个URL状态。") # <-- 添加的日志
    except FileNotFoundError:
        logging.warning(f"URL 状态文件 '{URL_STATES_PATH}' 未找到，使用空状态。") # <-- 添加的日志
    except json.JSONDecodeError as e:
        logging.error(f"解析 '{URL_STATES_PATH}' 的 JSON 失败: {e}，返回空状态。") # <-- 添加的日志
        return {}
        ...
        if 'last_checked' in state:
            try:
                ...
                if (current_time - last_checked_datetime).days < CONFIG['url_state']['expiration_days']:
                    ...
                    logging.debug(f"移除过期 URL 状态: {url}（最后检查于 {state['last_checked']}）") # <-- 添加的日志
            except ValueError:
                logging.warning(f"无法解析 URL {url} 的 last_checked 时间戳: '{state['last_checked']}'，保留状态。") # <-- 添加的日志
                updated_url_states[url] = state
        else: # 没有 last_checked 字段的旧状态也保留
            updated_url_states[url] = state
    logging.info(f"清理过期状态后，剩余 {len(updated_url_states)} 个有效URL状态。") # <-- 添加的日志
...
def save_url_states_local(url_states):
    ...
    try:
        ...
        logging.info(f"已将 {len(url_states)} 个URL状态保存到 '{URL_STATES_PATH}'。") # <-- 添加的日志
    except Exception as e:
        logging.error(f"保存 URL 状态到 '{URL_STATES_PATH}' 失败: {e}") # <-- 添加的日志
...
def fetch_url_content_with_retry(url, url_states):
    ...
    if CONFIG['url_state']['cache_enabled'] and url in content_cache:
        logging.debug(f"从缓存读取 URL 内容: {url}") # <-- 添加的日志
    ...
    if 'etag' in current_state:
        ...
        logging.debug(f"为 {url} 添加 If-None-Match 头: {current_state['etag']}") # <-- 添加的日志
    if 'last_modified' in current_state:
        headers['If-Modified-Since'] = current_state['last_modified']
        logging.debug(f"为 {url} 添加 If-Modified-Since 头: {current_state['last_modified']}") # <-- 添加的日志
    ...
    try:
        ...
        if response.status_code == 304:
            logging.debug(f"URL 内容未变更 (304 Not Modified): {url}") # <-- 添加的日志
            if url not in url_states: # 确保即使是旧的URL也能更新last_checked
                url_states[url] = {}
            url_states[url]['last_checked'] = datetime.now().isoformat()
            return None # 内容未变更，返回None

        content = response.text
        content_hash = hashlib.md5(content.encode('utf-8')).hexdigest()

        if 'content_hash' in current_state and current_state['content_hash'] == content_hash:
            logging.debug(f"URL 内容未变更（内容哈希相同）: {url}") # <-- 添加的日志
            if url not in url_states:
                url_states[url] = {}
            url_states[url]['last_checked'] = datetime.now().isoformat()
            return None # 内容哈希相同，返回None
        ...
        if CONFIG['url_state']['cache_enabled']:
            ...
            with open(cache_file, 'w', encoding='utf-8') as f:
                f.write(content)
            logging.debug(f"URL内容已写入缓存文件: {cache_file}") # <-- 添加的日志

        logging.debug(f"成功获取并更新URL内容: {url}") # <-- 添加的日志
        return content
    except requests.exceptions.RequestException as e:
        logging.error(f"请求 URL 失败（重试后）: {url} - {e}") # <-- 添加的日志
        return None # 原始代码这里是 `return None` 而不是 `raise`
    except Exception as e:
        logging.error(f"获取 URL 内容未知错误: {url} - {e}") # <-- 添加的日志
        return None # 原始代码这里是 `return None` 而不是 `raise`
...
def pre_screen_url(url):
    ...
    if not isinstance(url, str) or not url:
        logging.debug(f"预筛选过滤（无效类型或空）: {url}") # <-- 添加的日志
        return False
    
    # 检查URL是否包含协议头
    if not re.match(r'^[a-zA-Z0-9+.-]+://', url):
        logging.debug(f"预筛选过滤（无有效协议）: {url}") # <-- 添加的日志
        return False

    # 检查是否包含非法字符或空格
    if re.search(r'[^\x00-\x7F]', url) or ' ' in url:
        logging.debug(f"预筛选过滤（包含非法字符或空格）: {url}") # <-- 添加的日志
        return False
    ...
    try:
        ...
        if parsed_url.scheme not in CONFIG['url_pre_screening']['allowed_protocols']:
            logging.debug(f"预筛选过滤（不支持的协议）: {url} (协议: {parsed_url.scheme})") # <-- 添加的日志
            return False
        
        # 检查网络位置（域名或IP）是否存在
        if not parsed_url.netloc:
            logging.debug(f"预筛选过滤（无网络位置）: {url}") # <-- 添加的日志
            return False
        ...
        for pattern in compiled_invalid_url_patterns:
            if pattern.search(url):
                logging.debug(f"预筛选过滤（匹配无效模式 '{pattern.pattern}'）: {url}") # <-- 添加的日志
                return False
        
        # 检查URL长度
        # 原始代码是 `if len(url) < 15:` 这里保持原始逻辑
        if len(url) < 15: 
            logging.debug(f"预筛选过滤（URL 过短，长度 {len(url)} < 15）: {url}") # <-- 添加的日志
            return False
        
        return True
    except ValueError as e:
        logging.debug(f"预筛选过滤（URL 解析错误）: {url} - {e}") # <-- 添加的日志
        return False
...
def filter_and_modify_channels(channels):
    ...
    logging.info(f"URL 预筛选后剩余 {pre_screened_count} 个频道，进一步过滤和修改后得到 {len(filtered_channels)} 个频道。") # <-- 添加的日志
...
def check_http_url(url, timeout):
    ...
    try:
        ...
        if not is_valid:
            logging.debug(f"HTTP URL 检查失败: {url} (Status: {response.status_code})") # <-- 添加的日志
        return is_valid
    except requests.exceptions.RequestException as e:
        logging.debug(f"HTTP URL 检查失败: {url} - {e}") # <-- 添加的日志
        return False
...
def check_rtmp_url(url, timeout):
    ...
    try:
        ...
        logging.warning("ffprobe 未找到或不可用，跳过 RTMP 检查。请安装 ffmpeg/ffprobe。") # <-- 添加的日志
        return False
    ...
    try:
        ...
        if not is_valid:
            logging.debug(f"RTMP URL 检查失败: {url} (ffprobe return code: {result.returncode}, stderr: {result.stderr.decode()})") # <-- 添加的日志
        return is_valid
    except subprocess.TimeoutExpired:
        logging.debug(f"RTMP URL 检查超时: {url}") # <-- 添加的日志
        return False
    except Exception as e:
        logging.debug(f"RTMP URL 检查错误: {url} - {e}") # <-- 添加的日志
        return False
...
def check_rtp_url(url, timeout):
    ...
    try:
        ...
        if not host or not port:
            logging.debug(f"RTP URL 格式错误（无主机或端口）: {url}") # <-- 添加的日志
            return False
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            logging.debug(f"RTP URL (UDP) 简单可达性检查通过: {url}") # <-- 添加的日志
            return True
    except socket.timeout:
        logging.debug(f"RTP URL (UDP) 检查超时: {url}") # <-- 添加的日志
        return False
    except socket.error as e:
        logging.debug(f"RTP URL (UDP) 检查失败: {url} - {e}") # <-- 添加的日志
        return False
    except Exception as e:
        logging.debug(f"RTP URL 检查错误: {url} - {e}") # <-- 添加的日志
        return False
...
def check_channel_validity(channel_tuple, validation_config):
    ...
    if scheme in ['http', 'https']:
        ...
        logging.debug(f"跳过不支持协议的频道验证: {name}, {url} (协议: {scheme})") # <-- 添加的日志
        return channel_tuple, False # 不支持的协议直接返回无效

    if not is_valid:
        logging.debug(f"频道 '{name}' (URL: {url}) 被标记为无效。") # <-- 添加的日志
...
def validate_channels_concurrently(channels, validation_config):
    ...
    if max_workers == -1: # -1 表示使用所有可用CPU核心
        max_workers = os.cpu_count() or 1
    logging.info(f"开始并发验证 {len(channels)} 个频道，使用 {max_workers} 个工作线程。") # <-- 添加的日志
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(future_to_channel), total=len(channels), desc="验证频道有效性"):
            ...
            try:
                ...
                logging.error(f"验证频道 {channel_tuple} 时发生错误: {e}") # <-- 添加的日志

    logging.info(f"频道验证完成。总共 {len(channels)} 个频道，其中 {len(valid_channels)} 个有效。") # <-- 添加的日志
...
def get_github_trending(search_keywords):
    ...
    for keyword in search_keywords:
        # 这里使用 search/repositories 而不是 search/code，因为它更适合查找项目本身
        ...
        try:
            logging.info(f"正在从GitHub API搜索代码: {search_url}") # <-- 添加的日志
            ...
            logging.info(f"从GitHub代码搜索获取到 {len(search_results)} 个与关键词 '{keyword}' 相关的代码项，从中提取 {len(all_repos)} 个唯一仓库URL。") # <-- 添加的日志
        except requests.exceptions.RequestException as e:
            logging.error(f"从GitHub代码搜索获取数据失败 (关键词: {keyword}): {e}") # <-- 添加的日志
        time.sleep(1) # 避免触发GitHub API限速
    return list(set(all_repos)) # 确保最终返回的仓库URL是唯一的
...
def get_m3u_from_github_repo(repo_url):
    ...
    try:
        ...
        if len(path_parts) < 2:
            logging.warning(f"无法从仓库URL解析所有者和名称: {repo_url}") # <-- 添加的日志
            return []
        
        repo_owner = path_parts[0]
        repo_name = path_parts[1]

        # 尝试获取仓库默认分支（master/main）的根目录内容
        contents_api_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/contents"
        headers = {"Authorization": f"token {GITHUB_TOKEN}"}
        
        logging.debug(f"尝试获取仓库 {repo_url} 的内容列表: {contents_api_url}") # <-- 添加的日志
        ...
        for item in contents:
            if item['type'] == 'file' and (item['name'].lower().endswith(('.m3u', '.m3u8', '.txt', '.conf')) or 'readme' in item['name'].lower()):
                potential_raw_urls.append(item['download_url']) # 使用 download_url 获取原始文件内容

        for file_url in potential_raw_urls:
            try:
                logging.debug(f"尝试从文件URL {file_url} 获取内容查找m3u8链接。") # <-- 添加的日志
                ...
                if file_content_response.status_code == 200:
                    ...
                    logging.debug(f"从 {file_url} 找到 {len(found_links)} 个m3u8链接。") # <-- 添加的日志
                    if found_links: # 如果找到，可以提前结束对该仓库的搜索，除非需要所有链接
                        pass # 这里选择不break，继续查找所有潜在文件
            except requests.exceptions.RequestException as e:
                logging.debug(f"访问文件 {file_url} 失败: {e}") # <-- 添加的日志
        
    except requests.exceptions.RequestException as e:
        logging.error(f"从GitHub仓库 {repo_url} 获取文件内容失败: {e}") # <-- 添加的日志
    except Exception as e:
        logging.error(f"处理GitHub仓库 {repo_url} 失败: {e}") # <-- 添加的日志
    return list(set(m3u_urls)) # 去重
...
def load_channels_from_local_files(file_paths):
    ...
    for file_path in file_paths:
        try:
            ...
            logging.info(f"从本地文件加载频道成功: {file_path}, 数量: {len(all_channels)}") # <-- 添加的日志
        except FileNotFoundError:
            logging.warning(f"本地文件未找到: {file_path}") # <-- 添加的日志
        except Exception as e:
            logging.error(f"加载本地文件失败 {file_path}: {e}") # <-- 添加的日志
    return all_channels
...
def search_channels(search_keywords, backup_urls, local_file_paths, invalid_url_patterns):
    ...
    """
    all_channels = []
    source_tracker = {} # 用于追踪每个频道来自哪个原始URL，方便调试

    logging.info("开始从本地文件加载频道...") # <-- 添加的日志
    ...
    logging.info(f"已从本地文件加载 {len(local_channels)} 个频道。") # <-- 添加的日志

    logging.info("开始从GitHub Trending获取仓库URL...") # <-- 添加的日志
    github_repo_urls = get_github_trending(search_keywords)
    logging.info(f"从GitHub Trending获取到 {len(github_repo_urls)} 个仓库URL。") # <-- 添加的日志
    ...
    for repo_url in tqdm(github_repo_urls, desc="从GitHub仓库获取m3u链接"):
        ...
        for m3u_url in m3u_urls:
            channels_from_github = extract_channels_from_url(m3u_url, url_states, source_tracker) # 这里需要 url_states
            all_channels.extend(channels_from_github)
        time.sleep(0.1) # 避免API限速

    logging.info("开始从备用URL获取频道...") # <-- 添加的日志
    for url in tqdm(backup_urls, desc="从备用URL获取频道"):
        channels_from_url = extract_channels_from_url(url, url_states, source_tracker) # 这里需要 url_states
        all_channels.extend(channels_from_url)
        time.sleep(0.1) # 避免请求过快

    logging.info(f"所有源初步提取后总频道数: {len(all_channels)}") # <-- 添加的日志
    return all_channels # 返回包含所有源的原始频道列表
...
def remove_duplicate_channels(channels):
    ...
    """
    unique_channels = {} # 使用字典来辅助去重
    for name, url in channels:
        channel_key = (name.strip(), url.strip())
        if channel_key not in unique_channels:
            unique_channels[channel_key] = True # 标记为已添加
            
    result = list(unique_channels.keys())
    logging.info(f"频道去重完成。去重前 {len(channels)} 个，去重后 {len(result)} 个。") # <-- 添加的日志
    return result
    ...
    for category, keywords in category_keywords.items():
        for keyword in keywords:
            if keyword in clean_name:
                logging.debug(f"频道 '{channel_name}' 匹配到关键词 '{keyword}', 归类为 '{category}'") # <-- 添加的日志
                return category
    logging.debug(f"频道 '{channel_name}' 未匹配到任何分类关键词，归类为 '未分类'") # <-- 添加的日志
    return "未分类" # 默认未分类类别
...
def process_and_save_channels_by_category(all_channels, config):
    ...
    logging.info(f"临时分类文件将保存到目录: {channels_dir}") # <-- 添加的日志
    ...
    for category in config['channel_categories']['ordered_categories'] + ["未分类"]:
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
            logging.debug(f"已清空旧的分类文件: {file_path}") # <-- 添加的日志
        logging.info(f"准备处理分类 '{category}', 对应文件: {file_path}") # <-- 添加的日志

    # 清空未分类文件 (如果它应该独立存在且路径不同于其他分类文件)
    # 原始代码里有单独处理 UNCATEGORIZED_CHANNELS_FILE
    if os.path.exists(uncategorized_file_path):
        os.remove(uncategorized_file_path)
        logging.debug(f"已清空旧的未分类文件: {uncategorized_file_path}") # <-- 添加的日志
    logging.info(f"准备处理未分类频道文件: {uncategorized_file_path}") # <-- 添加的日志
    ...
    for channel_name, channel_url in tqdm(all_channels, desc="处理频道分类"):
        ...
        logging.debug(f"频道 '{channel_name}' 已添加到 '{category}' 分类列表") # <-- 添加的日志

    # 将分类好的频道写入各自的临时文件
    for category, channels in categorized_channels.items():
        if channels: # 只有当这个分类下有频道时才写入文件
            ...
            if file_path:
                try:
                    with open(file_path, 'a', encoding='utf-8') as f:
                        for name, url in channels:
                            f.write(f"{name},{url}\n")
                    logging.info(f"已将 {len(channels)} 个频道写入文件: {file_path}") # <-- 添加的日志
                except Exception as e:
                    logging.error(f"写入文件 {file_path} 失败: {e}") # <-- 添加的日志
            else:
                logging.warning(f"未找到分类 '{category}' 对应的文件路径。") # <-- 添加的日志
        else:
            logging.info(f"分类 '{category}' 中没有频道，跳过文件写入。") # <-- 添加的日志
...
def merge_local_channel_files(config):
    ...
    with open(IPTV_LIST_PATH, 'w', encoding='utf-8') as f:
        ...
        logging.info(f"已清空并初始化文件: {IPTV_LIST_PATH}。") # <-- 添加的日志
    ...
    logging.info(f"频道合并顺序：{all_categories_to_merge}") # <-- 添加的日志
    ...
    for category in all_categories_to_merge:
        ...
        if os.path.exists(file_path):
            try:
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    logging.info(f"开始合并分类 '{category}' 的频道文件: {file_path}") # <-- 添加的日志
                    for line in f:
                        line = line.strip()
                        if ',' in line:
                            name, url = line.split(',', 1)
                            channel_key = (name.strip(), url.strip()) # 使用元组作为唯一标识
                    ...
                    if channels_in_current_category: # 只有当这个分类有新频道时才写入标题和频道
                        ...
                        logging.info(f"已将 {len(channels_in_current_category)} 个来自 '{category}' 分类的频道合并到 {IPTV_LIST_PATH}。") # <-- 添加的日志
                    else:
                        logging.info(f"分类 '{category}' 文件中没有新的频道可合并。") # <-- 添加的日志

            except Exception as e:
                logging.error(f"合并文件 {file_path} 失败: {e}") # <-- 添加的日志
        else:
            logging.info(f"分类文件 '{file_path}' 不存在，跳过合并。") # <-- 添加的日志
...
def main():
    ...
    # 这里需要将 url_states 传递给 search_channels 函数，因为其内部的 extract_channels_from_url 会用到
    global url_states # 声明为全局变量以便在 search_channels 中使用
    ...
    os.makedirs(CHANNELS_DIR, exist_ok=True) # 确保目录存在

    # 7. 处理并保存分类频道到临时文件
    logging.info("步骤 7: 处理并保存分类频道到临时文件...")
    process_and_save_channels_by_category(valid_channels, CONFIG) # 使用valid_channels
