...
def load_config():
    ...
    if content:
        try:
            ...
            return {
                'search_keywords': ['extension:m3u in:file', 'iptv playlist extension:m3u,m3u8 in:file'],
                'per_page': 100,
                'max_search_pages': 5,
                ...
                'ordered_categories': [],
                'url_pre_screening': {'allowed_protocols': ['http', 'https', 'rtmp', 'rtp', 'p3p'], 'invalid_url_patterns': []},
                'requests_pool_size': 50,
                'requests_retry_total': 3,
                'requests_retry_backoff_factor': 1,
                'channel_check_workers': 50
            }
    logging.error(f"无法从 GitHub 的 '{CONFIG_PATH_IN_REPO}' 加载配置，使用默认配置。")
    return {
        'search_keywords': ['extension:m3u in:file', 'iptv playlist extension:m3u,m3u8 in:file'],
        'per_page': 100,
        'max_search_pages': 5,
