...
def fetch_from_github(file_path_in_repo):
    ...
    try:
        ...
        logging.info(f"成功从 GitHub 获取 {file_path_in_repo}。")
    ...
    try:
        with open(file_name, 'w', encoding='utf-8') as file:
            for item in data_array:
                file.write(item + '\n')
        logging.info(f"数据成功写入 '{file_name}'。")
    except Exception as e:
            ...
            if "#genre#" not in line and "," in line and "://" in line:
                ...
                if '#' in channel_address_raw:
                    ...
                    if channel_url:
                        extracted_channels.append((channel_name, channel_url))
                        channel_count += 1
        logging.info(f"成功从 URL：{url} 中提取 {channel_count} 个频道。")
    except Exception as e:
...
def filter_and_modify_channels(channels):
    ...
    logging.info(f"URL 预筛选后，剩余 {pre_screened_count} 个频道等待进一步过滤。")
...
def auto_discover_github_urls(urls_file_path_remote, github_token):
        ...
        while page <= MAX_SEARCH_PAGES:
            ...
            try:
                ...
                if not data.get('items'):
                    logging.info(f"在关键词 '{keyword}' 的第 {page} 页上未找到更多结果。")
                    break
                ...
                logging.info(f"关键词 '{keyword}'，第 {page} 页搜索完成。当前已发现 {len(found_urls)} 个原始 URL。")
    ...
    if new_urls_count > 0:
        ...
        logging.info(f"成功发现并添加了 {new_urls_count} 个新的 GitHub IPTV 源 URL 到 {urls_file_path_remote}。总 URL 数：{len(updated_urls)}")
    else:
...
def main():
    ...
    try:
        if os.path.exists('iptv.txt'):
            os.remove('iptv.txt')
            logging.info(f"已删除临时文件 'iptv.txt'。")
        if os.path.exists('iptv_speed.txt'):
            os.remove('iptv_speed.txt')
            logging.info(f"已删除临时文件 'iptv_speed.txt'。")
    except OSError as e:
