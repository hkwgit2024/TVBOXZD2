...
# 配置日志，根据 config.yaml 的 log_level 动态设置
def setup_logging(log_level):
    level = {
        ...
        "ERROR": logging.ERROR
    }.get(log_level.upper(), logging.ERROR)
...
def filter_and_modify_channels(channels):
    ...
    for name, url in channels:
        ...
        if any(word.lower() in name.lower() for word in NAME_FILTER_WORDS):
            continue
        modified_name = name
        for pattern, new_str in replacement_patterns:
            modified_name = pattern.sub(new_str, modified_name)
...
def group_and_limit_channels(lines):
    ...
    for line_content in lines:
        ...
        if line_content:
            channel_name, url = line_content.split(',', 1)
    ...
    final_grouped_lines = []
    for channel_name in sorted(grouped_channels):
        sorted_channels = sorted(grouped_channels[channel_name], key=lambda x: x[0])
        for _, ch_line in sorted_channels[:MAX_CHANNEL_URLS_PER_GROUP]:
...
def main():
    ...
    for channel_line in channels_for_matching:
        ...
        if channel_name not in all_template_channel_names:
            if any(keyword in channel_name for keyword in movie_keywords):
                output_file_path = os.path.join(local_channels_directory, '电影_iptv.txt')
