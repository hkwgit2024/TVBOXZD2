...
PER_PAGE = 100
MAX_SEARCH_PAGES = 2  # 减少分页数量以降低请求频率
        ...
        if url.startswith("http"):
            try:
                ...
                async with session.get(url, timeout=timeout) as response:
                    response.raise_for_status()
                    content_length = 0
                    max_content_to_download = 2 * 1024 * 1024
                    async for chunk in response.content.iter_chunked(1024 * 1024):
                        content_length += len(chunk)
                        if content_length >= max_content_to_download:
                            break
                    elapsed_time_download = (time.time() - start_time) * 1000
                    ...
                    if download_speed_mbps < 0.5:
                        logging.info(f"URL {url} 下载速度过低 ({download_speed_mbps:.2f} Mbps)，判定为无效。")
                        return None, False

                    if url.endswith(('.m3u8', '.m3u')):
                        proc = await asyncio.create_subprocess_exec(
                            'ffprobe', '-v', 'error', '-show_streams', '-print_format', 'json', '-timeout', str(timeout * 1000000), url,
            ...
            try:
                ...
                logging.info(f"RTMP URL {url} 检查异常: {e}")
                return None, False
            ...
            try:
                ...
                logging.info(f"P3P URL {url} 检查失败: {e}")
                return None, False
        else:
            logging.info(f"不支持的协议: {url}")
            return None, False
    except Exception as e:
        logging.info(f"检查 URL {url} 流质量时发生未知错误: {e}")
        return None, False
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            ...
            if '#genre#' in header:
                ...
                for channel_name in grouped_channels:
                    for ch_line in grouped_channels[channel_name][:50]:
...
async def main():
    os.makedirs(CONFIG_DIR, exist_ok=True)
    os.makedirs("地方频道", exist_ok=True)
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [process_url_async(url, updated_last_modified_cache, session, blacklist, semaphore) for url in urls_to_crawl]
    ...
    async with aiohttp.ClientSession() as session:
        check_tasks = [check_stream_quality(url, session) for name, url in filtered_channels]
        total_checks = len(check_tasks)
        completed_checks = 0
        for i, future in enumerate(asyncio.as_completed(check_tasks)):
    ...
    valid_channels_results = sorted(valid_channels_results)
