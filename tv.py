...
def save_to_github(file_path_in_repo, content, commit_message):
    ...
    try:
        ...
        response.raise_for_status()
        logging.info(f"[{datetime.now()}] 保存 {file_path_in_repo} 成功")
        return True
    except requests.exceptions.RequestException as e:
...
def save_url_states_remote(url_states, retention_days=7, max_size_mb=50):
    ...
    try:
        ...
        for url, state in url_states.items():
            ...
            if last_checked and last_checked > cutoff_date and fail_count < 10:
        ...
        if check_file_size(content, file_path_in_repo):
            save_to_github(file_path_in_repo, content, "更新 URL 状态")
...
def save_keyword_stats(keyword_stats, retention_days=7, max_size_mb=50):
    ...
    try:
        ...
        save_to_github(file_path_in_repo, content, "更新关键词统计")
    ...
    try:
        ...
        for channel_id, data in channel_cache.items():
            existing_cache[channel_id] = {
                ...
                'last_checked': current_time, 'is_valid': data.get('is_valid', False)
            }
        ...
        save_to_github(file_path_in_repo, content, "更新频道缓存")
...
def load_config():
    ...
    if content:
        try:
            ...
            logging.info(f"[{datetime.now()}] 配置加载成功")
            return config
...
def read_txt_to_array_local(file_name):
    ...
    try:
        with open(file_name, 'r', encoding='utf-8') as file:
            ...
            logging.info(f"[{datetime.now()}] 读取 {file_name}，{len(lines)} 行")
            return lines
...
def read_existing_channels(file_path):
    ...
    try:
        ...
        logging.info(f"[{datetime.now()}] 读取 {file_path}，{len(existing_channels)} 个频道")
...
def write_sorted_channels_to_file(file_path, data_list):
    ...
    try:
        ...
        logging.info(f"[{datetime.now()}] 追加 {len(all_channels - existing_channels)} 个频道到 {file_path}")
    except Exception as e:
...
def get_url_file_extension(url):
    ...
    try:
        ...
        logging.warning(f"[{datetime.now()}] 获取 {url} 扩展名失败：{e}")
        return ""
...
def clean_url_params(url):
    ...
    try:
        ...
        logging.error(f"[{datetime.now()}] 清理 {url} 失败：{e}")
        return url
...
def load_url_states():
    ...
    if content:
        try:
            ...
            logging.info(f"[{datetime.now()}] 加载 {len(states)} 个 URL 状态")
            return states
...
def load_channel_cache():
    ...
    if content:
        try:
            ...
            logging.info(f"[{datetime.now()}] 加载 {len(cache)} 个频道缓存")
            return cache
        except json.JSONDecodeError as e:
...
def fetch_url_content_with_retry(url, url_states):
    ...
    try:
        ...
        if response.status_code == 304:
            logging.info(f"[{datetime.now()}] {url} 未修改 (304)")
        ...
        if 'content_hash' in current_state and current_state['content_hash'] == content_hash:
            logging.info(f"[{datetime.now()}] {url} 哈希相同")
            url_states[url] = url_states.get(url, {})
            url_states[url]['last_checked'] = datetime.now().isoformat()
            return None
        ...
        logging.info(f"[{datetime.now()}]] {url} 获取成功")
        return content
...
def extract_channels_from_url(url, url_states, channel_cache):
    """从 URL 提取频道"""
    logging.debug(f"[{datetime.now()}] 提取 {url} 的频道")
    try:
        ...
        for line in lines:
            ...
            if "#genre#" not in line and "," in line and "://" in line:
                parts = line.split(',', 1)
                if len(parts) != 2:
                    logging.warning(f"[{datetime.now()}] 跳过无效行：{line}")
                    continue
                ...
                if not re.match(r'^[a-zA-Z0-9+.-]+://', channel_address_raw):
                    logging.warning(f"[{datetime.now()}]] 无效 URL：{channel_address_raw}")
                    continue
                ...
                if '#' in channel_address_raw:
                    ...
                    for channel_url in url_list:
                        channel_url = clean_url_params(channel_url)
                    ...
                    if channel_url and pre_screen_url(channel_url):
                        extracted_channels.append((channel_name, channel_url))
                        channel_count += 1
        logging.info(f"[{datetime.now()}] 从 {url} 提取 {channel_count} 个频道")
...
def pre_screen_url(url):
    ...
    try:
        ...
        for pattern in in [re.compile(p, re.IGNORECASE) for p in invalid_url_patterns]:
...
def filter_and_modify_channels(channels):
    ...
    logging.info(f"[{datetime.now()}] 过滤后 {len(filtered_channels)} 个频道")
...
def check_rtmp_url(url, timeout=5):
    """检查 RTMP URL"""
    try:
...
def check_channel_validity_and_speed(channel_name, url, url_states, channel_cache, timeout=CHANNEL_CHECK_TIMEOUT):
    ...
    try:
        ...
        if is_valid:
            ...
            logging.info(f"[{datetime.now()}] {channel_name} ({url}) 有效，耗时 {elapsed_time:.2f}ms")
...
def check_channels_multithreaded(channel_lines, url_states, channel_cache, max_workers=50):
    """多线程检查频道"""
    logging.info(f"[{datetime.now()}] 检查 {len(channel_lines)} 个频道，线程 {max_workers}")
    results = []
    checked_count = 0
    total = len(channel_lines)
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if checked_count % 100 == 0:
                logging.info(f"[{datetime.now()}] 已检查 {checked_count}/{total} 个")
            try:
...
def group_and_limit_channels(lines):
    ...
    for name in sorted(grouped.keys()):
        final_lines.extend(line + '\n' for line in grouped[name][:MAX_CHANNEL_URLS_PER_GROUP])
    return final_lines
...
def merge_local_channel_files(local_channels_directory, output_file_name="iptv_list.txt"):
    ...
    final_lines = generate_update_time_header() if not existing_channels else []
    ...
    all_channels = existing_channels | new_channels
    for name, url in sorted(all_channels):
        if (name, url) not in existing_channels:
            final_lines.append(f"{name},{url}\n")

    try:
        with open(output_file_name, "w", encoding='utf-8') as file:
            file.writelines(final_lines)
        logging.info(f"[{datetime.now()}] 合并完成，输出到 {output_file_name}")
    except Exception as e:
...
def read_txt_to_array_remote(file_path_in_repo):
    ...
    logging.info(f"[{datetime.now()}] 读取 {file_path_in_repo}，{len(lines)} 行")
    return lines
...
# --- GitHub URL 发现 ---
def auto_discover_github_urls(file_path_in_repo, github_token, keyword_stats):
    """发现 GitHub URL"""
    logging.info(f"[{datetime.now()}] 开始发现 GitHub URL")
    if not github_token:
    ...
    keyword_counts = {keyword.lower(): 0 for keyword in SEARCH_KEYWORDS}

    for i, keyword in enumerate(SEARCH_KEYWORDS):
        logging.info(f"[{datetime.now()}] 处理 '{keyword}'，第 {i+1}/{len(SEARCH_KEYWORDS)}")
        ...
        while page <= MAX_SEARCH_PAGES:
            ...
            try:
                response = requests.get(f"{GITHUB_API_BASE_URL}{SEARCH_CODE_ENDPOINT}", 
                                      headers=headers, timeout=GITHUB_API_TIMEOUT, params=params)
                response.raise_for_status()
                ...
                if not data.get('items'):
                    logging.info(f"[{datetime.now()}] '{keyword}' 无结果，第 {page} 页")
                    break
        ...
        keyword_counts[keyword.lower()] = len(keyword_found_urls)
        logging.info(f"[{datetime.now()}]] '{keyword}' 找到 {len(keyword_found_urls)} 个 URL")

    save_keyword_stats(keyword_counts)

    for keyword, count in sorted(keyword_counts.items()):
        logging.info(f"[{datetime.now()}] 关键词 '{keyword}'：{count} 个 URL")
        if count <= 3:
            logging.warning(f"[{datetime.now()}] 低效 '{keyword}' ({count})")
    ...
    if new_urls_count:
        ...
        write_array_to_txt_to_remote(file_path_in_repo, updated_urls, "更新 urls.txt")
        logging.info(f"[{datetime.now()}] 添加 {new_urls_count} 个新 URL 到 {file_path_in_repo}")
    else:
        logging.info(f"[{datetime.now()}] 无 无新 URL")
...
def main():
    ...
    try:
        ...
        if not auto_discover_github_urls(URLS_PATH_IN_REPO, GITHUB_TOKEN, keyword_stats):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(extract_channels_from_url, url, url_states, channel_cache) for url in urls: url}
            for future in as_completed(futures):
                try:
        ...
        logging.info(f"[{datetime.now()}] 提取 {len(all_extracted_channels)} 个频道")
        ...
        logging.info(f"[{datetime.now()}]") {len(valid_channels)} 个有效频道")
        ...
        for template_file in template_files:
            ...
            matched_channels = [ch for ch in channels_to_process if ch.split(',', 1)[0].strip() in template_channel_names]

            if "CCTV" in template_name.upper():
                matched_channels = sort_cctv(matched_channels)
                logging.info(f"[{datetime.now()}] {template_name} 已排序")
            ...
            with open(output_file, 'a', encoding='utf-8') as f:
                ...
                for name, url in sorted(all_channels):
                    if (name, url) not in existing_channels:
                        f.write(f"{name},{url}\n")
            logging.info(f"[{datetime.now()}] 添加 {len(new_channels)} 个频道到 {output_file}")
        ...
        if channels_to_process:
            unmatched_channels = [ch for ch in channels_to_process if ch.split(',', 1)[0].strip() not in all_template_channel_names]
        ...
        with open(unmatched_output_file, 'a', encoding='utf-8') as f:
            for name, url in sorted(all_unmatched):
                if (name, url) not in existing_unmatched:
                    f.write(f"{name},{url}\n")
        logging.info(f"[{datetime.now()}] 保存 {len(all_unmatched)} 个未匹配到 {unmatched_output_file}")

        # 清理临时文件
        for temp_file in ['iptv_speed.txt']:
            if os.path.exists(temp_file):
                os.remove(temp_file)
                logging.info(f"[{datetime.now()}] 删除 {temp_file}")

        logging.info(f"[{datetime.now()}] 所有任务完成")
        return True

    except Exception as e:
        logging.error(f"[{datetime.now()}] 主程序错误：{e}")
        return False

def sort_cctv(channels):
    """排序 CCTV 频道"""
    def sort_key(ch):
