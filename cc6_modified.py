...
System: 64解码失败: {url}")
            ...
            core_keys = ['add', 'port', 'id', 'type']
            optional_keys = ['net', 'tls']  # 保留 net 和 tls，移除其他次要字段
            clean_config = {}
            ...
            clean_config['ps'] = urllib.parse.unquote(config.get('ps', ''))[:20] or 'node'
            ...
            fragment = decode_base64(core_part_match.group(2).split('#')[-1])[:20] if '#' in core_part_match.group(2) else 'node'
        ...
        if isinstance(json_content, list):
            for config_dict in json_content:
                if isinstance(config_dict, dict) and 'id' in config_dict:
                    clash_vmess_proxy = {
                        "type": "vmess",
                        "name": config_dict.get('ps', 'node')[:20],
                    ...
                    if protocol_type in [p for p in NODE_PATTERNS.keys()]:
                        ...
                        for user_config in users:
                            proxy_cfg = {
                                "type": protocol_type,
                                "name": user_config.get('id', user_config.get('email', 'node'))[:20],
...
async def process_urls(domains: Set[str], max_concurrency: int, timeout: int, use_browser: bool) -> Tuple[Dict[str, Set[str]], Dict, Set]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for result in results:
            if isinstance(result, tuple) and len(result) == 2:
                domain, nodes = result
                url_to_nodes[domain] = nodes
            elif isinstance(result, Exception):
                logger.error(f"处理域名时发生异常: {result}")
    ...
    for domain, nodes in url_to_nodes.items():
        ...
        url_to_nodes[domain] = sorted(list(fingerprint_to_node.values()))
        all_nodes_collected.update(url_to_nodes[domain])

    logger.info(f"去重前节点数: {len(all_nodes_collected)}, 去重后节点数: {sum(len(nodes) for nodes in url_to_nodes.values())}")
...
def main():
    args = setup_argparse()
    logger.info(f"命令行参数: sources={args.sources}, nodes_output_dir={args.nodes_output_dir}, stats_output={args.stats_output}, max_concurrency={args.max_concurrency}, timeout={args.timeout}, chunk_size_mb={args.chunk_size_mb}, use_browser={args.use_browser}")
    ...
    total_nodes_extracted = sum(len(nodes) for nodes in url_to_nodes.values())
    logger.info(f"前10个节点样本: {list(url_to_nodes.values())[0][:10] if url_to_nodes else []}")
    ...
    avg_node_length_bytes = 50 if not total_nodes_extracted else sum(len(node.encode('utf-8')) for nodes in url_to_nodes.values() for node in nodes) // total_nodes_extracted
    ...
    for url, nodes in url_to_nodes.items():
        if not nodes:
            logger.info(f"URL {url} 没有提取到节点，跳过保存。")
            continue
        safe_filename = generate_safe_filename(url)
        base_output_path = os.path.join(output_dir, safe_filename)
        total_nodes = len(nodes)
        
        if total_nodes == 0:
            continue
        elif total_nodes <= max_nodes_per_file:
            try:
                content = '\n'.join(nodes)
                ...
                with open(base_output_path, 'w', encoding='utf-8') as f:
                    f.write(content)
                file_size_mb = os.path.getsize(base_output_path) / (1024 * 1024)
                logger.info(f"保存 {total_nodes} 个节点到 {base_output_path} ({file_size_mb:.2f} MB)")
            except Exception as e:
                logger.error(f"保存节点文件 '{base_output_path}' 失败: {e}")
                num_files = max(1, (total_nodes + min_nodes_per_file - 1) // min_nodes_per_file)
                estimated_lines_per_file = (total_nodes + num_files - 1) // num_files
                current_node_idx = 0
                for i in range(num_files):
                    end_node_idx = min(current_node_idx + estimated_lines_per_file, total_nodes)
                    nodes_for_file = nodes[current_node_idx:end_node_idx]
                    output_path = f"{base_output_path[:-4]}_part_{i + 1:03d}.txt"
                    try:
                        content = '\n'.join(nodes_for_file)
