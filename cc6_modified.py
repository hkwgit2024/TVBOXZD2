...
async def process_single_url_strategy(session: aiohttp.ClientSession, url: str, timeout: int, use_browser: bool, browser_context: Optional[BrowserContext] = None) -> Set[str]:
    ...
    nodes = set(extract_nodes(content)) if content else set()
    logger.debug(f"URL {url} 提取到 {len(nodes)} 个节点")
    return nodes
...
async def process_urls(domains: Set[str], max_concurrency: int, timeout: int, use_browser: bool) -> Tuple[Dict[str, Set[str]], Dict, Set]:
    ...
    logger.info(f"总共提取到 {len(all_nodes_collected_overall)} 个唯一节点 (跨所有源)。")
    logger.info(f"协议统计 (所有源): {dict(protocol_counts)}")
    
    return url_to_nodes, url_node_counts, failed_urls

def main():
    args = setup_argparse()
    logger.info(f"命令行参数: sources={args.sources}, nodes_output_dir={args.nodes_output_dir}, stats_output={args.stats_output}, max_concurrency={args.max_concurrency}, timeout={args.timeout}, use_browser={args.use_browser}")
    ...
    report_lines = [
        ...
        f"总共提取到 {total_nodes_extracted_overall} 个节点 (每个源的节点已内部去重)。",
        "\n每个源 URL 的节点提取数量:"
    ]
    ...
    for url_domain, count in sorted(url_node_counts.items(), key=lambda x: x[1], reverse=True):
        status = "成功" if count > 0 else ("失败" if url_domain in failed_urls else "无节点")
        report_lines.append(f"{url_domain:<70} {count:<15} {status:<10}")
    ...
    try:
        ...
        with open(args.stats_output, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for url_domain, count in sorted(url_node_counts.items(), key=lambda x: x[1], reverse=True):
                status = "成功" if count > 0 else ("失败" if url_domain in failed_urls else "无节点")
                writer.writerow({'Source_URL': url_domain, 'Nodes_Found': count, 'Status': status})
