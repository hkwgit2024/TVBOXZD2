...
log_debug "Created temp file for merged nodes: $MERGED_NODES_TEMP_FILE"
ALL_TEST_RESULTS_TEMP_FILE=$(mktemp)
log_debug "Created temp file for test results: $ALL_TEST_RESULTS_TEMP_FILE"
CURRENT_RUN_SUCCESS_TEMP_FILE=$(mktemp)
log_debug "Created temp file for success results: $CURRENT_RUN_SUCCESS_TEMP_FILE"
CURRENT_RUN_FAILED_TEMP_FILE=$(mktemp)
log_debug "Created temp file for failed results: $CURRENT_RUN_FAILED_TEMP_FILE"

# 仅在脚本成功退出时清理临时文件
trap 'if [ $? -eq 0 ]; then rm -f "$MERGED_NODES_TEMP_FILE" "$ALL_TEST_RESULTS_TEMP_FILE" "$CURRENT_RUN_SUCCESS_TEMP_FILE" "$CURRENT_RUN_FAILED_TEMP_FILE" node_batch_*; fi' EXIT
...
test_single_node() {
    ...
    if [[ "$NODE_LINK" =~ ^(hysteria2|vless|trojan):\/\/(.+@)?([0-9a-zA-Z.-]+|\[[0-9a-fA-F:]+\]):([0-9]+) ]]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ -z "$HOSTNAME_OR_IP" ] || [ -z "$PORT" ]; then
                log WARN "$LOG_PREFIX - 无法从 vmess JSON 解析 add 或 port: $NODE_LINK_ORIGINAL"
                echo "FAILED:$NODE_LINK_ORIGINAL"
                return
            fi
        else
            log WARN "$LOG_PREFIX - 无法解码 vmess 链接: $NODE_LINK_ORIGINAL"
            echo "FAILED:$NODE_LINK_ORIGINAL"
            return
        fi
    fi
    ...
    if [ $? -eq 0 ]; then
        echo "SUCCESS:$NODE_LINK_ORIGINAL"
    else
...
export -f test_single_node log log_debug
export LOG_FILE CONNECT_TIMEOUT DEBUG
...
BATCH_SIZE=$(( (TOTAL_UNIQUE_NODES + PARALLEL_JOBS - 1) / PARALLEL_JOBS ))
...
if ! git diff --quiet --exit-code "$SUCCESS_FILE" "$FAILED_FILE"; then
    ...
    if ! git commit -m "Update node connectivity results (automated by GitHub Actions)"; then
        ...
        if ! git pull --rebase origin "${GITHUB_REF##*/}"; then
            log WARN "Git pull --rebase 失败，尝试储藏更改..."
            git stash
            if ! git pull --rebase origin "${GITHUB_REF##*/}"; then
                log ERROR "Git pull --rebase 仍然失败，请检查远程分支或工作目录。"
                exit 1
            fi
            git stash pop || log WARN "储藏恢复失败，可能没有更改需要恢复。"
        fi
        
        if ! git push origin HEAD:${GITHUB_REF##*/}; then
            log ERROR "推送失败，请检查 Git 配置、网络连接或远程仓库权限。"
            exit 1
        fi
