...
while IFS= read -r NODE_LINK; do
    ...
    if [ -n "$HOSTNAME_OR_IP" ] && [ -n "$PORT" ]; then
        ...
        if [[ "$HOSTNAME_OR_IP" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ || "$HOSTNAME_OR_IP" =~ ^\[?[0-9a-fA-F:]+\]?$ ]]; then
            ...
            # dig 同时查询 A (IPv4) 和 AAAA (IPv6) 记录，取第一个结果
            ...
            if [ -n "$RESOLVED_IP" ]; then
                ...
                echo "$NODE_LINK" >> "$FAILED_FILE" # 记录为失败节点
                continue # 跳过当前节点，处理下一个
            fi
        fi
    else
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "$NODE_LINK" >> "$SUCCESS_FILE" # 将成功连接的完整节点链接保存到指定的输出文件
    else
        log WARN "  - 结果: 无法连接到 $IP:$PORT (可能被防火墙阻止或服务未运行)"
        echo "$NODE_LINK" >> "$FAILED_FILE" # 将失败的完整节点链接保存到失败文件
    fi
done < "$MERGED_NODES_TEMP_FILE" # 从合并后的临时文件读取
...
# 检查是否有更改
# 使用 --exit-code 可以让 git diff 在有差异时返回非零退出码
