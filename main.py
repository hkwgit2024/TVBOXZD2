...
def decode_proxy(proxy_url):
    ...
    try:
        ...
        print(f"Failed to decode proxy {proxy_url}: {e}")
        return None
...
def test_proxy(proxy):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Proxy {proxy['tag']} failed with status {response.status_code}")
            return None

    except (requests.RequestException, subprocess.SubprocessError) as e:
        print(f"Proxy {proxy.get('tag', 'unknown')} test failed: {e}")
        return None
    finally:
        try:
            process.terminate()
