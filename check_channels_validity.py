...
import yaml
import json # 导入json库以解析ffprobe的输出

# --- 配置和加载模块 ---
CONFIG_PATH = "config/config.yaml"
INPUT_CHANNELS_PATH = "output/iptv.txt"
VALID_CHANNELS_PATH = "output/valid_channels_temp.txt"
...
def check_rtmp_url_advanced(url, timeout):
    """
    检查 RTMP URL 是否可达，并尝试判断是否为直播流（而非录像）。
    通过 ffprobe 分析流的持续时间。
    """
    try:
        # 使用 -show_format 和 -show_streams 获取流的元数据
        cmd = ['ffprobe', '-v', 'quiet', '-timeout', str(timeout), '-print_format', 'json', '-show_format', '-i', url]
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout, text=True, encoding='utf-8')

        if result.returncode != 0:
            print(f"ffprobe 无法打开流: {url}")
            return False
        ...
        if duration_str:
            ...
            # 持续时间小于1小时（3600秒）或非常短的，通常是录像
            # 但有些直播也可能持续时间较短，这里设置一个合理的阈值
            if duration < 3600:
                print(f"警告: {url} 持续时间为 {duration:.2f} 秒，可能为录像。")
                return False

        # 如果没有 duration 字段，这通常是直播流
        return True

    except (subprocess.TimeoutExpired, json.JSONDecodeError, Exception) as e:
        print(f"处理 RTMP 流 {url} 时发生异常: {e}")
        return False
...
def check_channel_validity(channel, url_states, timeout):
    ...
    if protocol in ['http', 'https']:
        ...
        is_valid = check_rtmp_url_advanced(url, timeout)
