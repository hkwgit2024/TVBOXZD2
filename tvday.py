...
SEARCH_UPDATED_WITHIN_DAYS = CONFIG.get('search_updated_within_days', 30)  # 新增：搜索最近更新的天数
...
def auto_discover_github_urls(urls_file_path_remote, github_token):
    """自动从 GitHub 发现新的 IPTV 源 URL，仅限指定时间范围内更新的仓库。"""
    if not github_token:
    ...
    start_date = current_date - timedelta(days=SEARCH_UPDATED_WITHIN_DAYS)
    date_str = start_date.strftime('%Y-%m-%d')

    logging.warning(f"正在开始从 GitHub 自动发现新的 IPTV 源 URL（仅限 {date_str} 之后更新的仓库）...")
    ...
    for i, keyword in enumerate(SEARCH_KEYWORDS):
        ...
        while page <= MAX_SEARCH_PAGES:
            ...
            try:
                ...
                if not data.get('items'):
                    logging.debug(f"在关键词 '{keyword}' 的第 {page} 页上未找到更多结果（{date_str} 之后更新）。")
                    break
                    ...
                    if raw_url:
                        ...
                        if cleaned_url.startswith("https://raw.githubusercontent.com/") and \
                           ...
                           pre_screen_url(cleaned_url):
                            ...
                            logging.debug(f"已发现原始 GitHub URL（通过预筛选，{date_str} 之后更新）：{cleaned_url}")
                        else:
        ...
        logging.warning(f"关键词 '{keyword}' 找到 {keyword_url_counts[keyword]} 个有效 URL（{date_str} 之后更新）。")
    ...
    if low_or_no_result_keywords:
        logging.warning(f"\n建议从 config.yaml 的 search_keywords 中删除以下结果较少（≤{low_result_threshold}）或无结果的关键词，以节省时间和 API 请求：")
        for keyword, count in low_or_no_result_keywords:
            logging.warning(f"  - '{keyword}' （找到 {count} 个 URL）")
    else:
        logging.warning("所有关键词均有合理数量的搜索结果，无需删除。")
    ...
    if new_urls_count > 0:
        ...
        write_array_to_txt_remote(urls_file_path_remote, updated_urls, f"通过 GitHub 发现的新 URL 更新 urls.txt（{date_str} 之后更新）")
        logging.warning(f"成功发现并添加了 {new_urls_count} 个新的 GitHub IPTV 源 URL 到 {urls_file_path_remote}（{date_str} 之后更新）。总 URL 数：{len(updated_urls)}")
    else:
        logging.warning(f"未发现新的 GitHub IPTV 源 URL（{date_str} 之后更新）。")
...
def main():
    ...
    for channel_line in channels_for_matching:
        ...
        if channel_name not in all_template_channel_names:
            unmatched_channels_list.append(channel_name)
