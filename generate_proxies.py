...
def parse_vmess_node(line, name_counts, seen_nodes):
    ...
    try:
        ...
        if not cipher:  # Handle missing or empty cipher
            cipher = 'auto'
...
def parse_vless_node(line, name_counts, seen_nodes):
    ...
    try:
        ...
        if not host_port or ':' not in host_port:
            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                f.write(f"Skipping VLESS node {line[:50]}... (invalid host/port)\n")
            return None
        host, port = host_port.rsplit(':', 1)
        port = int(port)
        if not validate_server_port(host, port):
            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                f.write(f"Skipping VLESS node {line[:50]}... (invalid server/port)\n")
            return None
        node_key = ('vless', host, port, uuid)
        if node_key in seen_nodes:
            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                f.write(f"Skipping duplicate VLESS node {line[:50]}...\n")
            return None
        seen_nodes.add(node_key)
        ...
        with open(LOG_FILE, 'a', encoding='utf-8') as f:
            f.write(f"Error parsing VLESS node {line[:50]}...: {e}\n")
        return None
                ...
                for proxy in config['proxies']:
                    ...
                    if proxy['type'] == 'ss':
                        ...
                        if 'network' in proxy and proxy['network'] not in VALID_VLESS_NETWORKS:
                            stats['invalid'] += 1
                            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                                f.write(f"Skipping VLESS proxy {proxy.get('name', 'unnamed')} (unsupported network: {proxy['network']})\n")
                            continue
                        node_key = ('vless', proxy['server'], proxy['port'], proxy.get('uuid'))
                        if node_key in seen_nodes:
                            stats['duplicates'] += 1
                            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                                f.write(f"Skipping duplicate VLESS proxy {proxy.get('name', 'unnamed')}\n")
                            continue
                        seen_nodes.add(node_key)
                    else:
                        stats['invalid'] += 1
