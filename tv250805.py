...
def check_channels_multithreaded(channel_lines, url_states, max_workers=CONFIG['network']['channel_check_workers']):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor, tqdm(total=total_channels, desc="检查频道有效性", unit="ch") as pbar:
...
def merge_local_channel_files(local_channels_directory, output_file_name, url_states):
    ...
    with tqdm(total=len(files_to_merge_paths), desc="合并本地文件", unit="file") as pbar:
    ...
    # 按分类重新组织有效频道
    channels_with_names_urls = [(name, url) for _, line in valid_channels_from_check for name, url in [line.split(',', 1)]]
    categorized_channels_checked, uncategorized_channels_checked, final_ordered_categories_checked = categorize_channels(
        channels_with_names_urls
    )
    ...
    try:
        with open(output_file_name, "w", encoding='utf-8') as iptv_list_file:
            ...
            with tqdm(total=len(final_ordered_categories_checked), desc="写入主列表文件", unit="cat") as pbar:
...
def auto_discover_github_urls(urls_file_path_local, github_token):
    ...
    with tqdm(total=total_keywords, desc="GitHub URL 发现", unit="kw") as pbar_kw:
        for i, keyword in enumerate(CONFIG.get('search_keywords', [])):
            pbar_kw.set_description(f"GitHub URL 发现 - 关键词: '{keyword}'")
...
def cleanup_urls_local(urls_file_path_local, url_states):
    ...
    with tqdm(total=len(all_urls), desc="清理过期URL", unit="url") as pbar:
        for url in all_urls:
...
def process_and_save_channels_by_category(all_channels, url_states, source_tracker):
    ...
    with tqdm(total=len(final_ordered_categories) + 1, desc="按分类保存频道", unit="cat") as pbar:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=min(CONFIG['network']['url_fetch_workers'], 10)) as executor, tqdm(total=len(urls_to_process), desc="提取频道信息", unit="url") as pbar:
    ...
    try:
        logging.warning("开始清理临时文件")
        temp_files = ['iptv.txt', 'iptv_speed.txt']
        for temp_file in tqdm(temp_files, desc="清理临时文件", unit="file"):
        ...
        if os.path.exists(temp_dir):
            ...
            for f_name in tqdm(files_to_remove, desc="清理临时频道文件", unit="file"):
