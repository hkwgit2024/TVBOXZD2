        ...
        try:
            async with session.get(url, timeout=30) as response:
                if response.status != 200:
                    print(f"Failed to fetch proxies from {url}: HTTP {response.status}")
                    return []
...
def parse_proxy_line(line: str) -> Optional[Dict]:
    ...
    try:
        ...
        proxy = {'name': f'{scheme}_{url_parts.netloc}_{time.time()}'}

        if scheme == 'ss':
            # Shadowsocks: ss://user:pass@server:port#name
            auth_data = url_parts.netloc.split('@')
            if len(auth_data) != 2:
                return None
            proxy['type'] = 'ss'
            proxy['cipher'], proxy['password'] = auth_data[0].split(':')
...
async def main():
    ...
    parser.add_argument('--chunk', type=int, default=0, help='Chunk index for parallel testing')
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f"Testing chunk {args.chunk}/{args.total_chunks}: {len(proxies)} proxies")
        ...
        # 保存结果
        output_prefix = f"chunk_{args.chunk}" if args.total_chunks > 1 else ""
        ...
        print(f"Results saved to data/521{output_prefix}.yaml and data/invalid_nodes{output_prefix}.yaml")
