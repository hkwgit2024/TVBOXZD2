...
def parse_node_url_to_mihomo_config(node_url: str) -> Dict | None:
    ...
    try:
        ...
        if scheme == "ss":
            # Shadowsocks: ss://[cipher:password@]server:port 或 ss://base64_encoded
            if "@" not in netloc:
                try:
            ...
            try:
                decoded_str = base64.b64decode(node_url_clean[len("vmess://"):] + '==').decode('utf-8')
            ...
            print(f"警告: 未知或不支持的节点协议: {scheme} ({node_url})")
            return None
...
def validate_proxy(proxy: Dict, original_url: str, index: int) -> tuple[bool, str]:
    """验证代理节点格式，返回 (is_valid, error_message)"""
    ...
    for i in range(0, len(parsed_proxies), batch_size):
        ...
        for result in results:
            if isinstance(result, dict):
                ...
                print(f"{result['name']}: {result['status']}{'，延迟: %.2fms' % result['latency'] if result['latency'] else ''} (原始URL: {result['original_url']})")
            else:
